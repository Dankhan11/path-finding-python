...
print ('Welcome to A STAR pathfinding algorithm demo')
...
def make_grid(rows, width):
    grid = []
    gap = width // rows #int division gives gap between each row 
    for i in range (rows):
        grid.append([])
        for j in range(rows):
            node = Node(i, j, gap, rows) #creating a new instance 
            grid[i].append(node)
